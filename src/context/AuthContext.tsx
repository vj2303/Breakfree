...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchAssessmentCenters = async (page: number = 1, limit: number = 10, search: string = '') => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/assessment-centers?page=${page}&limit=${limit}&search=${search}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const updateAssessmentCenter = async (id: string, data: Record<string, unknown>) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3001/api/assessment-centers/${id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
        },

        ...
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                ...
                <div className="mb-4">
                  ...
                  {selectedAssignment.activities
                    ...
                    .map((activity) => (
                      <div key={activity.activityId} className="mb-3 p-2.5 bg-gray-50 border border-gray-200 rounded">
                        <div className="flex items-center justify-between mb-1.5">
                          <div>
                            <p className="font-medium text-sm text-black">{activity.activityDetail.name}</p>
                          ...
                          <div className="mt-2 space-y-1.5">
                            {activity.competency.subCompetencyNames.map((subComp, idx) => (
                              <div key={idx} className="flex items-center justify-between bg-white p-2 rounded border border-gray-200">
                                <label className="text-xs text-black">{subComp}</label>
                ...
                <div className="border-t border-gray-200 pt-3">
                  <h4 className="text-sm font-medium text-black mb-2">Overall Competency Assessment</h4>
                  {selectedAssignment.competencies.map((competency) => (
                    <div key={competency.id} className="mb-3">
                      <h5 className="font-medium text-sm text-black mb-1.5">{competency.competencyName}</h5>
                ...
                {competency.subCompetencyNames.map((subComp, index) => (
                        <div key={index} className="mb-2">
                          <div className="flex items-center justify-between bg-gray-50 p-2 rounded border border-gray-200">
                            <label className="text-xs font-medium text-black">{subComp}</label>
                            <div className="flex items-center gap-1.5">
                      <input
                        type="number"
                        min="0"
                        ...
                        step="0.5"
                                ...
                                className="w-20 px-2 py-1 border border-gray-300 rounded text-xs text-black focus:outline-none focus:ring-1 focus:ring-black focus:border-black"
                      />
                      <span className="text-xs text-gray-600">/10</span>
                            </div>
                    </div>
            ...
            <div className="border-t border-gray-200 pt-3">
              <div className="mb-0">
                <div className="flex items-center justify-between mb-2">
                  <p className="font-medium text-sm text-black">Overall Assessment</p>
                  <span className={`text-xs px-2 py-0.5 rounded border ${
                        scoreStatus[selectedAssignmentId] === 'SUBMITTED' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                  }`}>
                        {scoreStatus[selectedAssignmentId] || 'DRAFT'}
                  </span>
                </div>
                <div className="mb-3">
                  <p className="text-xs text-gray-600 mb-1.5">Overall Comments (Required)</p>
                  <textarea
                    rows={4}
                        value={comments[selectedAssignmentId] || ''}
                        onChange={(e) => setComments(prev => ({ ...prev, [selectedAssignmentId]: e.target.value }))}
                    className="w-full border border-gray-300 rounded p-2 text-sm text-black focus:outline-none focus:ring-1 focus:ring-black focus:border-black"
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="flex-1 bg-gray-100 hover:bg-gray-200 disabled:bg-gray-100 disabled:opacity-50 text-black px-3 py-2 rounded text-xs flex items-center justify-center gap-1.5 font-medium"
                  >
                    {isSubmittingScore ? (
                      <>
                        <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  ...
                  <button 
                        ...
                        disabled={isSubmittingScore || !comments[selectedAssignmentId]?.trim()}
                    className="flex-1 bg-black hover:bg-gray-800 disabled:bg-gray-400 text-white px-3 py-2 rounded text-xs flex items-center justify-center gap-1.5 font-medium"
                  >
                    {isSubmittingScore ? (
                      <>
                        <Loader2 className="h-3.5 w-3.5 animate-spin" />
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-base font-semibold text-black">Submissions & Activities</h3>
              ...
              return (
                <div className="space-y-3">
                  ...
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded">
                    <h4 className="font-semibold text-sm text-black mb-1.5">{selectedActivity.activityDetail.name}</h4>
                    <p className="text-xs text-gray-700 mb-1.5">{selectedActivity.activityDetail.description}</p>
                    <div className="flex items-center gap-2 text-xs">
                      <span className={`px-1.5 py-0.5 rounded border text-xs ${
                        sortedSubmissions.length > 0 ? 'bg-green-50 text-green-700 border-green-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                      }`}>
                    ...
                    {selectedActivity.activityDetail.instructions && (
                      <div className="mt-2 p-2 bg-white rounded border border-gray-200">
                        <p className="text-xs font-medium text-black mb-1">Instructions:</p>
                        <div 
                        ...
                        <div className="space-y-2.5">
                          {sortedSubmissions.map((sub: Submission & { submissionType?: string; submissionStatus?: string; fileUrl?: string; fileName?: string; fileSize?: number; textContent?: string; notes?: string }) => (
                            <div key={sub.id} className="bg-white border border-gray-200 rounded p-3">
                              <div className="flex justify-between items-start mb-1.5">
                                <div>
                                  <p className="font-semibold text-sm text-black">{selectedActivity.activityDetail.name}</p>
                                  <p className="text-xs text-gray-600 mt-0.5">
                              ...
                              {sub.submissionType === 'DOCUMENT' && sub.fileUrl && (
                                <div className="mt-2">
                                  <a 
                                    ...
                                    className="text-gray-700 hover:text-black flex items-center gap-1.5 text-xs"
                                  >
                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.414a2 2 0 00-2.828-2.828L9 10.172 7.586 8.586a2 2 0 10-2.828 2.828l4 4a2 2 0 002.828 0L16.828 9.828a2 2 0 000-2.828z" />
                                    </svg>

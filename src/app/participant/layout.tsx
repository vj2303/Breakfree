...
const sidebarLinks = [
  { label: 'Home', icon: '/home.svg', href: '/participant/dashboard' },
  { label: 'Assessment', icon: '/assessment.svg', href: '/participant/dashboard' },
  { label: 'Forum', icon: '/forum.svg', href: '/participant/forum' },
  { label: 'Calender', icon: '/calendar.svg', href: '/participant/calendar' },
];
  ...
  return (
    <div className="min-h-screen flex bg-[#F7F9FC]">
      ...
      <aside className="w-64 bg-white border-r flex flex-col justify-between py-8 px-4 min-h-screen">
          ...
          <nav className="flex flex-col gap-2">
            {sidebarLinks.map(link => (
              <a
                key={link.label}
                href={link.href}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg font-medium text-gray-700 hover:bg-blue-50 transition-colors ${pathname === link.href ? 'bg-blue-100 text-blue-700' : ''}`}
              >
                {/* You can replace with real SVG icons if available */}
        ...
        <div className="mb-4">
          <a href="/logout" className="flex items-center gap-2 px-4 py-2 rounded-lg text-gray-500 hover:bg-gray-100">
            <span className="inline-block w-5 h-5">
              <Image src="/logout.svg" alt="Log Out" width={20} height={20} />
            </span>
            Log Out
          </a>
        </div>

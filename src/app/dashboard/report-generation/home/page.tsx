...
const HomePage = () => {
  ...
  const fetchAssessorScores = useCallback(async (page = 1, limit = 10) => {
    ...
    try {
      ...
      if (!data.success || !data.data) throw new Error(data.message || 'Failed to fetch assessor scores');
      ...
      data.data.scores.forEach((score) => {
        ...
        if (score.status === 'SUBMITTED' || score.status === 'FINALIZED') stats.assessed += 1;
  ...
  useEffect(() => {
    if (activeTab === 'assessors') fetchAssessorScores(pagination.currentPage, pagination.itemsPerPage);
  ...
  const handleSaveScore = async () => {
    ...
    try {
      ...
      if (!data.success) throw new Error(data.message || 'Failed to update score');
        ...
        if (compScores && typeof compScores === 'object') {
          ...
          subCompetencies.forEach((subComp: string) => {
            ...
            if (typeof s === 'object' && s !== null) {
              ...
              if ('application' in scoreObj && typeof scoreObj.application === 'number') {
  ...
  const drawRingChart = (doc: jsPDF, x: number, y: number, score: number, maxScore = 10) => {
    const radius = 18;
  ...
  const drawPatternBar = (doc: jsPDF, x: number, y: number, w: number, h: number) => {
    ...
    if (h > 0 && w > 0) {
        ...
        for (let i = 0; i < numLines; i++) {
            ...
            if (endX >= x && startX <= x + w) {
                ...
                if (clippedStartY >= y && clippedEndY <= y + h && clippedStartX <= clippedEndX) {
  ...
  const handleDownloadAssessorData = async (stat: AssessorStats, event: React.MouseEvent) => {
    ...
    if (stat.scores.length === 0) { setError('No assessments available'); return; }
    ...
    try {
      ...
      for (const score of stat.scores) {
        ...
        const res = await fetch('https://api.breakfreeacademy.in/api/report-structures/generate-from-assessment-center', {
              method: 'POST',
        ...
        if (res.ok) {
            ...
            content = formatReportContentForPDF(r.data.reportContent, score.participant.name, score.assessmentCenter.name, score);
        } else {
            // Fallback content if report gen fails
            content = formatReportContentForPDF({}, score.participant.name, score.assessmentCenter.name, score);
        }

        // --- PDF CONSTRUCTION ---

        if (pageNum > 1) { doc.addPage(); pageNum++; yPos = margin; }

        // COVER PAGE
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(...COLORS.BLACK);
        doc.text("ACME ASSESSMENTS", margin, yPos);
        doc.setFontSize(9);
        ...
        const introTxt = doc.splitTextToSize(content.introduction || `Report for ${score.participant.name}.`, pageWidth - margin * 2);
        ...
        if (content.comments.strengths.length > 0) {
            ...
            content.comments.strengths.slice(0, 4).forEach(s => {
                ...
                doc.setFontSize(10);
                doc.setFont('helvetica', 'normal');
                doc.text(doc.splitTextToSize(s, pageWidth - margin - 15), margin + 10, yPos);
                yPos += 10;
            });
            yPos += 10;
        }
        ...
        content.analysis.competencies.forEach((comp: Record<string, unknown>) => {
            ...
            if(descriptors.length > 0) {
                descriptors.forEach((d: string | Record<string, unknown>) => {
                    ...
                    doc.setFont('helvetica', 'normal');
                    doc.setTextColor(...COLORS.DARK_GRAY);
                    const dl = doc.splitTextToSize(desc, pageWidth - margin - 90);
  ...
  return (
    <div style={{ padding: '24px', background: '#f8fafd', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={() => setActiveTab('groups')} style={{ padding: '10px 20px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 500, background: activeTab === 'groups' ? '#374151' : '#f3f4f6', color: activeTab === 'groups' ? '#ffffff' : '#1a1a1a' }}>Groups</button>
          <button onClick={() => setActiveTab('assessors')} style={{ padding: '10px 20px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 500, background: activeTab === 'assessors' ? '#374151' : '#f3f4f6', color: activeTab === 'assessors' ? '#ffffff' : '#1a1a1a' }}>Assessors</button>
        </div>
        
        {activeTab === 'assessors' && (
          <div style={{ position: 'relative' }}>
            <input type="text" placeholder="Search Assessors..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid #ddd', width: '250px' }} />
          </div>
        )}
      </div>
      ...
      {/* --- ERRORS --- */}
      {(error || groupsError) && (
        <div style={{ padding: '16px', background: '#fee2e2', color: '#991b1b', borderRadius: '8px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between' }}>
          <span>{error || groupsError}</span>
          <button onClick={() => { setError(null); setGroupsError(null); }} style={{ border: 'none', background: 'none', cursor: 'pointer' }}>×</button>
        </div>
      )}

      {/* --- GROUPS VIEW --- */}
      {activeTab === 'groups' && (
        groupsLoading ? <div style={{textAlign:'center', padding:'40px'}}>Loading...</div> :
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
          {groupsWithMarks.map(group => (
            <div key={group.id} onClick={() => setSelectedGroup(group)} style={{ background: 'white', padding: '24px', borderRadius: '12px', border: '1px solid #e5e7eb', cursor: 'pointer', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
              <h3 style={{ fontSize: '18px', fontWeight: 600, margin: '0 0 8px 0' }}>{group.name}</h3>
              <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>Admin: {group.admin}</p>
              <p style={{ fontSize: '14px', color: '#6b7280', marginTop: '4px' }}>Members: {group.members?.length || group.participantIds?.length || 0}</p>
                            </div>
                          ))}
                        </div>
      )}
      ...
      {activeTab === 'assessors' && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {filteredStats.map(stat => (
            <div key={stat.assessor.id} onClick={() => setSelectedAssessor(stat)} style={{ background: 'white', padding: '24px', borderRadius: '12px', border: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
              <div>
                <h3 style={{ fontSize: '18px', fontWeight: 600, margin: '0 0 4px 0' }}>{stat.assessor.name}</h3>
                <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>{stat.assessor.email}</p>
        </div>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <div style={{ fontSize: '13px', background: '#eff6ff', color: '#1d4ed8', padding: '4px 12px', borderRadius: '12px' }}>Total: {stat.totalAllotted}</div>
                <div style={{ fontSize: '13px', background: '#f0fdf4', color: '#15803d', padding: '4px 12px', borderRadius: '12px' }}>Done: {stat.assessed}</div>
                    <button
                      onClick={(e) => handleDownloadAssessorData(stat, e)}
                      disabled={downloading === stat.assessor.id || stat.scores.length === 0}
                  style={{ padding: '8px 16px', background: downloading === stat.assessor.id ? '#9ca3af' : '#111', color: 'white', border: 'none', borderRadius: '6px', fontSize: '13px', cursor: 'pointer' }}>
                  {downloading === stat.assessor.id ? 'Generating...' : 'Download PDF'}
                    </button>
                  </div>
                </div>
          ))}
      ...
      {/* --- MODAL: GROUP DETAILS --- */}
      {selectedGroup && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setSelectedGroup(null)}>
          <div style={{ background: 'white', width: '100%', maxWidth: '800px', borderRadius: '12px', padding: '24px', maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '24px', fontWeight: 700, margin: 0 }}>{selectedGroup.name}</h2>
              <button onClick={() => setSelectedGroup(null)} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer' }}>&times;</button>
            </div>

            <div style={{ marginBottom: '24px', padding: '16px', background: '#f9fafb', borderRadius: '8px' }}>
                <p style={{ margin: '0 0 8px 0' }}><strong>Admin:</strong> {selectedGroup.admin} ({selectedGroup.adminEmail})</p>
                <p style={{ margin: 0 }}><strong>Total Members:</strong> {selectedGroup.members?.length || selectedGroup.participantIds?.length || 0}</p>
            </div>

            <h3 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '16px' }}>Assessor Progress</h3>
            {selectedGroup.assessorMarks && selectedGroup.assessorMarks.length > 0 ? (
                <div style={{ display: 'grid', gap: '12px' }}>
                    {selectedGroup.assessorMarks.map((mark, i) => (
                        <div key={i} style={{ border: '1px solid #e5e7eb', padding: '16px', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                                <div style={{ fontWeight: 600 }}>{mark.assessorName}</div>
                                <div style={{ fontSize: '12px', color: '#6b7280' }}>evaluating {mark.participantName}</div>
                        </div>
                            <div style={{ fontSize: '13px', fontWeight: 500, color: mark.completedAssessments === mark.totalAssessments ? 'green' : 'orange' }}>
                ...
                <div style={{ textAlign: 'center', padding: '30px', color: '#6b7280' }}>No assessor activity found for this group.</div>
                      )}
          </div>
        </div>
      )}
      ...
      {selectedAssessor && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setSelectedAssessor(null)}>
          <div style={{ background: 'white', width: '100%', maxWidth: '900px', borderRadius: '12px', padding: '24px', maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '24px', fontWeight: 700, margin: 0 }}>Assessments: {selectedAssessor.assessor.name}</h2>
              <button onClick={() => setSelectedAssessor(null)} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer' }}>&times;</button>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {selectedAssessor.scores.length === 0 ? (
                <p style={{ textAlign: 'center', padding: '20px', color: '#6b7280' }}>No assessments assigned.</p>
              ) : (
                selectedAssessor.scores.map(score => (
                  <div key={score.id} style={{ border: '1px solid #e5e7eb', padding: '16px', borderRadius: '8px', background: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                      <div style={{ fontWeight: 600, fontSize: '16px' }}>{score.participant.name}</div>
                      <div style={{ fontSize: '13px', color: '#6b7280' }}>{score.assessmentCenter.name}</div>
                      <div style={{ fontSize: '12px', marginTop: '4px' }}>
                        Status: <span style={{ fontWeight: 600, color: score.status === 'FINALIZED' ? 'green' : score.status === 'SUBMITTED' ? 'blue' : 'orange' }}>{score.status}</span>
                      </div>
                      </div>
                      <button
                        onClick={() => handleEditScore(score)}
                      style={{ padding: '8px 16px', background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 500 }}>
                      Edit / View
                      </button>
                  </div>
                ))
      ...
      {showEditModal && selectedScore && editData && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => !saving && setShowEditModal(false)}>
          <div style={{ background: 'white', width: '100%', maxWidth: '800px', borderRadius: '12px', padding: '24px', maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: 700, margin: 0 }}>Edit Assessment: {selectedScore.participant.name}</h2>
              <button onClick={() => !saving && setShowEditModal(false)} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer' }}>&times;</button>
            </div>
            ...
            <div style={{ marginBottom: '20px' }}>
               <label style={{ display: 'block', fontSize: '14px', fontWeight: 600, marginBottom: '8px' }}>Overall Comments</label>
               <textarea 
                 value={editData.overallComments}
                 onChange={(e) => setEditData({...editData, overallComments: e.target.value})}
                 rows={4}
                 style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #d1d5db' }}
               />
            </div>

            <h3 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '12px', borderBottom: '1px solid #eee', paddingBottom: '8px' }}>Competency Scores</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                {Object.keys(editData.competencyScores).map(compId => (
                    <div key={compId} style={{ background: '#f9fafb', padding: '16px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                        <div style={{ fontWeight: 600, marginBottom: '12px', fontSize: '14px' }}>{compId}</div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px', gap: '10px', alignItems: 'center' }}>
                            {Object.keys(editData.competencyScores[compId]).map(subComp => (
                                <React.Fragment key={subComp}>
                                    <div style={{ fontSize: '13px', color: '#374151' }}>{subComp}</div>
                        <input
                          type="number"
                                        min="0" max="10" 
                    ...
                    <div style={{ marginTop: '12px' }}>
                             <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '4px' }}>Specific Comments</label>
                             <input 
                                type="text"
            ...
            <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button
                ...
                disabled={saving}
                    style={{ padding: '10px 20px', background: '#f3f4f6', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
                Cancel
              </button>
              <button

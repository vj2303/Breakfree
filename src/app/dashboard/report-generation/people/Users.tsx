...
const UsersComponent: React.FC = () => {
  ...
  const fetchAssessors = React.useCallback(async (page = 1, search = '') => {
    ...
    setError(null);
    try {
      const url = new URL('http://localhost:3000/api/assessors');
  ...
  const createOrUpdateAssessor = async () => {
    ...
    try {
      ...
      if (isEdit && editingAssessorId) {
        response = await fetch(`http://localhost:3000/api/assessors/${editingAssessorId}`, {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${getAuthToken()}`,
        ...
        response = await fetch('http://localhost:3000/api/assessors', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${getAuthToken()}`,
            'Content-Type': 'application/json',
  ...
  const sendBulkEmail = async () => {
    ...
    setEmailStatus(null);
    try {
      const response = await fetch('http://localhost:3000/api/assessors/send-bulk-login-credentials', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${getAuthToken()}`,
          'Content-Type': 'application/json',
  ...
  const deleteAssessor = async (id: string) => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`http://localhost:3000/api/assessors/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${getAuthToken()}`,
        },

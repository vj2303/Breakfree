...
const ParticipantsComponent: React.FC<ParticipantsComponentProps> = (props) => {
  ...
  const updateParticipant = async (id: string, participantData: Partial<Omit<Participant, 'id'>>) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/participants/${id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${getAuthToken()}`,
          'Content-Type': 'application/json',
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      console.log('⏰ Search timeout executed');
      if (currentPage === 1) {
  ...
  const sendBulkEmail = async () => {
    ...
    setEmailStatus(null);
    try {
      const response = await fetch('http://localhost:3000/api/participants/send-bulk-login-credentials', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${getAuthToken()}`,
          'Content-Type': 'application/json',
